...
import com.zhonghuilv.codegenerator.model.Column;
import com.zhonghuilv.codegenerator.model.ForeignKey;
import com.zhonghuilv.codegenerator.model.Table;
import com.zhonghuilv.codegenerator.util.Cache;
import com.zhonghuilv.codegenerator.util.ListHashtable;
import com.zhonghuilv.codegenerator.util.StringHelper;
import com.zhonghuilv.codegenerator.util.Utility;
...
public class DbModelProvider {
    ...
    private Connection getConnection(String dbname) throws SQLException {
        if (connection == null || connection.isClosed()) {
            String template = PropertiesProvider.getProperty("jdbc.url.template");
            Map<String, String> map = new HashMap<>();
            map.put("dbname", dbname);
            String uri = Utility.strFormatUsingDict(template, map);
            System.err.println("uri：" + uri);
            connection = DriverManager.getConnection(uri, PropertiesProvider
                    .getProperty("jdbc.username"), PropertiesProvider.getProperty("jdbc.password"));
        }
        return connection;
    }
    ...
    private Table createTable(Connection conn, ResultSet rs) throws SQLException {
        ...
        String tableRemark = rs.getString("REMARKS");//表注释
        ...
        //如果有忽略前缀
        String property = PropertiesProvider.getProperty("ignore.prefix");
        if (property != null && !"".equals(property.trim())) {
            table.setClassName(StringHelper
                    .makeAllWordFirstLetterUpperCase(realTableName.replaceFirst(property, "")));

            table.setMapping(realTableName.replaceFirst(property, ""));
        }
    ...
    private List getTableColumns(Table table, List primaryKeys, List indices, Map uniqueIndices, Map uniqueColumns)
        ...
        ListHashtable fklist = table.getImportedKeys().getAssociatedTables();
        List fkList = fklist.getOrderedValues();
        for (int t = 0; t < fkList.size(); t++) {
            ForeignKey fk = (ForeignKey) fkList.get(t);
            List fktable = fk.getParentColumns().getOrderedValues();
            for (int l = 0; l < fktable.size(); l++) {
                fkMap.put(fktable.get(l), "fk");
            }
        }

        String nogenerate = PropertiesProvider.getProperty("nogenerate");
        while (columnRs.next()) {
            ...
            boolean isfk = false;
            if ("fk".equals(fkMap.get(columnName))) {
                isfk = true;
            }
            //ylf 除掉不用生成的字段
            if (nogenerate != null && nogenerate.indexOf(columnName) > -1) continue;
            ...
            //remarks=(String)java.net.URLEncoder.encode(remarks, "UTF-8");
            ...
            Column column = new Column(
                    ...
                    isIndexed,
                    isUnique,
                    columnDefaultValue, remarks, isfk);
    ...
    private String getSynonymOwner(String synonymName) {
        ...
        String ret = null;
        try {
            ps = getConnection().prepareStatement("select table_owner from sys.all_synonyms where table_name=? and " +
                    "owner=?");
    ...
    public List<Table> getAllTables(String dbname) throws SQLException {
        List<Table> cache = Cache.getCache(dbname);
        if (cache != null)
            return cache;
        List<Table> allTables = getAllTables(getConnection(dbname));
        Cache.cache(dbname, allTables);
        return allTables;
    }
